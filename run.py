    ...
    def Bottomface_caculate(self):
        ...
        if self.main.crosssect_cam_state:
            try:
                ...
                pass
            try:
                self.main.CrossWorker.image_ready.disconnect(lambda loaded_image: save_lower_countour(loaded_image,SAVE_ROI=False))
            except TypeError:
                pass
...
class MainForm(Ui_mainWindow):
    ...
    def flowrate_OpenCamera(self):
        ...
        self.flowrate_cam.ExposureTime.set(500000.0)   # 曝光时间设置
    ...
    def flowrate_param_control(self):
        ...
        """弹出相机参数设置对话框"""
        # 确保相机已打开
        if not hasattr(self, 'flowrate_cam') or self.flowrate_cam is None:
        ...
        # -- 曝光时间 --
        label_exp = QLabel("曝光时间 (ms):", dialog)
        self._exp_input = QSpinBox(dialog)
        # 从相机读取当前曝光时间，作默认值
        try:
            current_exp = self.flowrate_cam.ExposureTime.get()
        except Exception:
            current_exp = 10.0
        self._exp_input.setRange(1, 10000)
        self._exp_input.setValue(int(current_exp))
        layout.addWidget(label_exp)
        layout.addWidget(self._exp_input)

        # -- 自动白平衡 --
        self._awb_checkbox = QCheckBox("自动白平衡", dialog)
        # 默认根据相机当前状态
        try:
            awb_on = bool(self.flowrate_cam.BalanceWhiteAuto.get())
        except Exception:
            awb_on = True
        self._awb_checkbox.setChecked(awb_on)
        layout.addWidget(self._awb_checkbox)
    ...
    def apply_flowratecamera_params(self, dialog):
        """读取对话框输入并设置相机参数"""
        exposure_time = self._exp_input.value()
        auto_wb = self._awb_checkbox.isChecked()
        ...
        try:
            ...
            # 日志输出
            self.flowInfo.append(f"✔ 曝光时间已设为 {exposure_time} ms")
    ...
    def apply_crosssectcamera_params(self, dialog):
        ...
        try:
            ...
            # 日志输出
            self.crossInfo.append(f"✔ 曝光时间已设为 {exposure_time} ms")
    ...
    def flowrate_Choose_ROI(self):
        try:
            self.flowWorker.start_capture.disconnect(self.flowWorker.bgrimage_capture)
        except TypeError:
            pass
        try:
            self.flowWorker.image_ready.disconnect(lambda bgr_image:self.flowViewerWorker.serial_flowrate_Choose_ROI(bgr_image))
        except TypeError:
            pass
    ...
    def measurement(self, method="Trad"):
        ...
        try:
            ...
            pass
        if method == "Trad":
            interval_ms = int(1000/self.framerate_get)
        if method == "xFeat":
            interval_ms = int(1000 / self.framerate_get)
