...
class TrafficWorker(QObject):
    ...
    def traffic_caculate(self):
        Q = self.main.v_t * self.main.area # 当前瞬时流量 (m^3/s)
        if self.main.Q_prev == 0.0:
            self.main.Q_prev = Q
        m_Q = 0.5 * (self.main.Q_prev + Q)    #线性插值积分
        self.main.Q_prev = Q


        elapsed = time.time() - self.main._start_ts
        self.main.traffic_plot_qchart.series.append(elapsed, m_Q)

        # 滚动 X 轴，保留最近 10s
        self.main.traffic_plot_qchart.axisX.setRange(max(0, elapsed - 10), elapsed)

        # 更新 Y 轴的最大值
        if m_Q > self.main.traffic_plot_qchart.axisY.max():
            self.main.traffic_plot_qchart.axisY.setMax(m_Q * 1.1)
...
class Simplified_MainForm(Simplified_MainWindow):
    ...
    def crosssect_measurement(self):
        ...
        self.CrossLoopWorker.start(interval_ms=int(60))
