...
class QTextEditLogger(logging.Handler, QtCore.QObject):
    ...
    def emit(self, record):
        """
         当有日志记录时，logging.Handler 会调用 emit 方法。
         这里将日志格式化成字符串，然后通过 signal_log 信号发送，
         让它到主线程中处理（线程安全）。
         """
...
def do_initialization():
    ...
    global device, xfeat
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")   # 用显卡运行神经网络模型
    ...
    # 用于液面分割的设置
    # colormap用于分割结果的颜色转换：即分割出的液面部分转为红色，其余部分转为黑色
    # trasnform用于分割网络数据预处理
...
class FlowrateCaptureWorker(QObject):
    ...
    def process_flowrate(self):
        ...
        if self.main.flowrate_cam_state:
            ...
            self.pixmap_ready.emit(pixmap) #采集结束，传递pixmap并发送信号

    @pyqtSlot()
    def bgrimage_capture(self):
        """
            该函数用于采集所有涉及流速计算所需的单帧图像
            """
        if self.main.flowrate_cam_state:
            # 采集 + 转成 bgr_image ——
            self.main.flowrate_cam.stream_on()
            ...
            self.image_ready.emit(bgr_image) #采集结束，传递bgr_image并发送信号
...
class FlowrateLoopWorker(QObject):
    ...
    def start(self, method,   interval_ms=33):
        ...
        try:
            ...
            pass
        # 只在这里做一次连接：image_ready 触发计算，保证在主线程里调用一次 serial_measurement
        ...
        self._timer.timeout.connect(self.flow_caculate) #周期调用flow_caculate ，flow_caculate发送采集信号，采集结束发送计算信号
    ...
    def flow_caculate(self):
        ...
        try:
            ...
            # 每帧时间开始记录，流速统计时间是该帧流速实际获取所花费的时间，而不是FlowrateLoopWorker循环周期时间
    ...
    def serial_measurement(self, bgr_image: ndarray, method):
        """计算单帧流速的函数，根据选择的传统方法或深度学习方法发送信号"""
        ...
        output = output_choose_vedio(coor, bgr_image)   # 裁剪框选区域
        vis = output.copy()
...
class FlowrateViewerWorker(QObject):
    ...
    def flowrate_Camera(self, pixmap: QPixmap):
        """
        接收到子线程传来的 QPixmap，用于更新单帧/多帧显示的视频帧
        """
        # 获取图像的宽高比，使其适应显示窗口
        Width = self.main.flowrate_cam.Width.get()
        Height = self.main.flowrate_cam.Height.get()
    ...
    def serial_flowrate_Choose_ROI(self, bgr_image: ndarray):
        ...
        get_choose_action(bgr_image, OnMouseAction) # 选择区域ROI
    ...
    def caculate_view(self, bgr_image):
        """
              该函数用于可视化涉及计算的帧、绘制速度图表、保存速度数据、流速预测
              """
        Width = self.main.flowrate_cam.Width.get()
        Height = self.main.flowrate_cam.Height.get()
        ...
        cv2.putText(bgr_image, "Operating_time: " + str(round(self.main.cost_time, 3)) + "ms", (50, 150), font, 1,
                    (0, 255, 0), 2)
        if self.main.uimode == 0:  #如果是用户界面，在调用可视化函数时发送流量计算信号并调用预测流速
            try:
        ...
        if self.main.frame_idx % self.main.iters == 0 and self.main.frame_idx != 0:
            ...
            self.main.df.to_csv("result/realtime_flowrate.csv", index=False)
...
class FlowrateCaculateWorker(QObject):
    ...
    def trad_caculate(self, output, vis):
        ...
        if len(self.main.tracks) > 0:  # 检测到角点后进行光流跟踪
            ...
            p0 = np.float32([tr[-1] for tr in self.main.tracks]).reshape(-1, 1, 2)  # 前一帧的角点
            ...
            for tr, (x, y), good_flag in zip(self.main.tracks, p1.reshape(-1, 2), good):  # 将跟踪正确的点列入成功跟踪点
                ...
                dis = math.sqrt(math.pow(tr[-1][1] - tr[-2][1], 2) + math.pow(tr[-1][0] - tr[-2][0], 2))    # 两帧之间特征点位移
            ...
            if 1 < len(self.main.tracks) < 100:
                ...
                self.main.v = self.main.d_ave / (self.main.curr_time - self.main.prev_time) #当前帧开始时间减上一帧开始时间，即上一帧实际花费时间
    ...
    def xFeat_caculate(self, output, vis):
        ...
        new_pt1 = []    # 用于储存当前帧特征点
        new_pt2 = []    # 用于储存前一帧特征点
        output = cv2.GaussianBlur(output, (15, 15), 0)  # 对ROI框选区域图像进行高斯滤波
        current = xfeat.detectAndCompute(output, top_k=1024)[0] # 输入特征提取网络

        # kp表示特征点坐标，des表示特征点描述子
...
class CrossViewerWorker(QObject):
    ...
    def crosssect_plt(self, area: float):
        ...
        更新截面积图表并保存截面积文件
        """
        self.main.crosssect_plot_qchart.y = area
...
class TrafficWorker(QObject):
    ...
    def traffic_caculate(self):
        ...
        # 5. 绘图
        self.main.traffic_plot_qchart.y = m_Q
        # 6. 保存到 DataFrame
        self.main.traffic.append(m_Q)
        self.main.traffic_times.append(datetime.datetime.now().strftime('%Y-%m-%d %H-%M-%S'))
        # 将新行添加到 DataFrame
        new_rows = pd.Series(
            {"时间": datetime.datetime.now().strftime('%Y-%m-%d %H-%M-%S'), "流量": round(m_Q, 2)}
        )
        self.main.df_traffic = pd.concat([self.main.df_traffic, new_rows.to_frame().T], ignore_index=True)
        print("保存流量", time.time())
        # 保存 DataFrame 到 CSV 文件
        self.main.df_traffic.to_csv("result/realtime_traffic.csv", index=False)
...
class Simplified_MainForm(Simplified_MainWindow):
    ...
    def __init__(self, Simplified_MainWindow):
        ...
        self.flowrate_plot_qchart.setTitle("测量流速")
        ...
        self.traffic_plot_qchart.setTitle("测量流量")
        ...
        # 加载ARIMA_Qchart波形界面
        self.arima_plot_qchart = arima_QChartViewPlot()
        self.arima_plot_qchart.setTitle("流速预测")
        self.arima_plot_qchart.setMargins(QMargins(20, 20, 20, 20))
        self.simple_ARIMA_plot_view.setChart(self.arima_plot_qchart)
        self.simple_ARIMA_plot_view.setRenderHint(QPainter.Antialiasing)  # 抗锯齿
        self.simple_ARIMA_plot_view.setRubberBand(QChartView.RectangleRubberBand)
    ...
    def flowrate_CloseCamera(self):
        ...
        self.flowrate_cam.stream_off() #关闭相机
    ...
    def crosssect_measurement(self):
        ...
        #设定周期时间
        interval_ms = int(1000 / self.crosscamframerate_get)
...
class MainForm(Ui_mainWindow):
    ...
    def __init__(self, MainForm):
        ...
        self.return_ui.clicked.connect(self.return_clicked)#返回用户界面
    ...
    def measurement(self, method="Trad"):
        ...
        self.flowWorker.start_capture.emit() #发送流速计算开始信号
